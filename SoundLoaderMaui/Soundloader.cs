...
static class Soundloader
{
    ...
    {
        Console.WriteLine($"{Tag} ExtractMp3Urls abs_path_m3u={abs_path_m3u}");
        ...
        try
        {
            ...
            if (!m3u.Exists())
            {
                Console.WriteLine($"{Tag} m3u file does not exist!");
                return urls;
            } else
            {
                Console.WriteLine($"{Tag} m3u file exists");
            }
            ...
            {
                Console.WriteLine($"{Tag} m3u line: {line}");
                // every non-comment line is the start of a new url
                if (!line.StartsWith("#"))
                {
                    urls.Add(line);
                }
    ...
    {
        Console.WriteLine($"{Tag} starting ConcatMp3");
